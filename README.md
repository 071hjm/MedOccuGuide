...
如果你准备公开发布本项目，以下内容应优先确认是否可以公开：

- 原始职业健康体检数据，如 `data.pkl`
- 由真实业务数据生成的训练产物，如 `processed_data/`、`saved_models/`
- 本地数据库文件，如 `platform_data.db`
- 本地基础模型权重，如 `models/`
- LoRA 适配器权重，如 `LLaMA-Factory/saves/...`
- 运行日志、下载日志、安装日志

如果上述资产存在隐私、版权或合规限制，建议：

- 仅公开代码和文档
- 仅保留脱敏后的最小示例输入
- 使用“目录占位 + 下载说明”代替直接提交大文件
...
- 该账号仅适用于本地演示，不适合作为公开仓库或生产环境默认配置。
- 开源发布前，建议移除默认明文密码，改为环境变量、初始化脚本或首次启动创建管理员的方式。
...
本仓库当前仍保留了若干开发机路径和本地演示配置。若要正式开源，建议优先完成以下工作：

1. 将硬编码绝对路径改为相对路径或环境变量配置。
2. 将默认账号密码改为初始化配置，而不是直接写死在代码中。
3. 清理原始数据、大模型权重、数据库和日志文件。
4. 为无法公开的模型和数据提供“下载说明”而不是直接提交。
5. 为 RAG 知识库内容补充来源说明与授权确认。
6. 增加许可证、贡献说明和安全说明。
